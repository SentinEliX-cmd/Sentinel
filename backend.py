...
def video_feed(camera_id):
    """MJPEG video stream with frame dropping to prevent browser queuing"""
    if not session.get('logged_in'):
        return redirect(url_for('index'))
    ...
    def generate():
        ...
        while True:
            ...
            time.sleep(max(0.001, min_interval - (time.time() - now)))
...
@app.route('/api/rules')
def get_rules():
    """Lightweight rules list for camera modal"""
    if not session.get('logged_in'):
        return jsonify({'error': 'Not authorized'}), 401
    try:
        conn = get_db_connection()
        rows = conn.execute("SELECT id, name, active, camera_ids FROM alert_rules ORDER BY id").fetchall()
        conn.close()
        return jsonify([{
            'id':         row['id'],
            'name':       row['name'],
